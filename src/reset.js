...
class Reset extends Component {
	...
	change = e => {
        ...
        console.log('this.state.confirmpass',this.state.confirmpass);
	};
	...
	submit = e => {
        ...
        if(this.state.newpass==this.state.confirmpass)
        {
            if(this.state.newpass.length<8){
                ...
                console.log('final else is called')
                fetch('http://localhost:3993/password', {
			method: 'POST',
			headers: {
        ...
        else if(this.state.newpass=='' ||this.state.confirmpass==''|| this.state.newpass=='' && this.state.confirmpass=='')
        {
              ...
              <div className="content">
                <div className="content_rgt">
                  <div className="register_sec">
                    ...
                    <ul>
                      <li><span>Enter New Password</span><input type="password" name="newpass"
												value={this.state.name}
												onChange={this.change}
												ref="newpass"
												required /></li>
                      <li><span>Confirm Password</span><input type="password" name="confirmpass"
												value={this.state.name}
												onChange={this.change}
												ref="confirmpass"
												required /></li>
                      <li><input type="submit" defaultValue="Submit" onClick={this.submit}/></li>
                    </ul>
                  </div>
                </div>
